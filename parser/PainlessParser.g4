...
rstatement
    : IF LP expression RP trailer ( ELSE trailer | { p.GetTokenStream().LA(1) != PainlessParserELSE }? )                 # if
...
chain
    : primary postfix*
    | decltype postdot postfix*
    | stdarray ( postdot postfix* )?
		| initializedarray postfix*
    ;
...
stdarray
    : NEW TYPE ( LBRACE expression RBRACE )+ # newstandardarray
		;

initializedarray
    : NEW TYPE LBRACE RBRACE LBRACK ( expression ( COMMA expression )* )? RBRACK # newinitializedarray
		;
